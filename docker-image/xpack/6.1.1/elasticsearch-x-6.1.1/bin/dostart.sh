...
if [ -z "$PID" ]; then
    ...
    #if [ ! -f /opt/elasticsearch/bin/setup-password.lock ]; then
    #    /opt/elasticsearch/bin/setup-password >> /opt/qingcloud/app-agent/log/setup-password.log 2>&1 
    #    touch /opt/elasticsearch/bin/setup-password.lock
    #fi
